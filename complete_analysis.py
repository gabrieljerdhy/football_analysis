...
from utils import save_video, VideoFrameIterator, get_video_info, monitor_memory_usage
...
def complete_large_video_analysis(
    ...
    print(f"📹 Video: {video_info['total_frames']} frames, {video_info['duration_seconds']/60:.1f} min")
    ...
    tracker = Tracker("models/best.pt", enable_jersey_detection=False)  # Just for interpolation
    ...
    for frame_num, player_track in enumerate(tracks["players"]):
        if frame_num % 1000 == 0:
            ...
            print(f"  📊 Analysis progress: {progress:.1f}% ({frame_num}/{len(tracks['players'])})")
        ...
        if assigned_player != -1:
            ...
            # Detect goals
            if ball_position:
                pass_counter.detect_goal(ball_position, assigned_player, current_team, frame_num)
            ...
            tackle_counter.detect_tackles_and_interceptions(player_track, -1, None, frame_num)
    ...
    team_csv_path, player_csv_path = export_consolidated_goal_statistics(
        video_name, pass_counter, enhanced_goal_stats, 
        final_team_goals, final_player_goals, tackle_counter
    )
    ...
    with VideoFrameIterator(input_video_path, batch_size) as frame_iterator:
        for batch_frames in frame_iterator:
            ...
            if frames_written % 1000 == 0:
                ...
                print(f"  🎬 Video progress: {progress:.1f}% ({frames_written}/{video_info['total_frames']})")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--batch-size", type=int, default=50, help="Batch size for video processing")
