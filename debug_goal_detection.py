...
def debug_ball_tracking(video_path, max_frames=100):
    ...
    tracks = tracker.get_object_tracks(debug_frames, read_from_stub=False, stub_path=None)
        ...
        if 1 in ball_track and ball_track[1]:
            ...
            if ball_position:
                ...
                if frame_num < 10:  # Show first 10 frames
                    print(f"  Frame {frame_num}: Ball at {ball_position}, bbox: {ball_bbox}")
    ...
    if ball_positions:
        ...
        print(f"    X: {min(x_coords)} - {max(x_coords)} (width: {max(x_coords) - min(x_coords)})")
        print(f"    Y: {min(y_coords)} - {max(y_coords)} (height: {max(y_coords) - min(y_coords)})")
        ...
        if frame_num < 3:  # Show first 3 frames
            ...
            if keypoints:
                for name, data in keypoints.items():
                    print(f"    {name}: {data['position']} (conf: {data['confidence']:.2f})")
        ...
        if 1 in ball_track and ball_track[1]:
            ...
            if ball_position:
                ...
                if x < 100 and 300 < y < 600:
                    print(f"  ⚠️  Ball in left goal area at frame {frame_num}: {ball_position}")
                elif x > 1180 and 300 < y < 600:
                    print(f"  ⚠️  Ball in right goal area at frame {frame_num}: {ball_position}")
...
def create_debug_video(video_path, output_path, max_frames=100):
    ...
    for frame_num, frame in enumerate(debug_frames):
        ...
        cv2.rectangle(debug_frame, (1180, 300), (1280, 600), (0, 0, 255), 3)  # Right goal
        ...
        if 1 in tracks["ball"][frame_num] and tracks["ball"][frame_num][1]:
            ...
            if ball_position:
                ...
                cv2.putText(debug_frame, f"Ball: {ball_position}", 
                           (ball_position[0] + 15, ball_position[1] - 15),
                           cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 0), 2)
        
        # Draw frame number
        cv2.putText(debug_frame, f"Frame: {frame_num}", (10, 30),
                   cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
        
        # Draw goal areas info
        cv2.putText(debug_frame, "Left Goal", (10, 350), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
        cv2.putText(debug_frame, "Right Goal", (1100, 350), 
...
def main():
    ...
    parser.add_argument("--frames", "-f", type=int, default=100, help="Max frames to analyze")
