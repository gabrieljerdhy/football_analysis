...
def check_output_files():
    ...
    # Check output videos
    output_videos_dir = Path("data/output_videos")
    if output_videos_dir.exists():
        ...
        if video_files:
            for video in sorted(video_files):
                ...
                print(
                    f"   ✅ {video.name} ({size_mb:.1f} MB, {mod_time.strftime('%H:%M:%S')})"
                )
        else:
    ...
    if output_dir.exists():
        ...
        if csv_files:
            for csv_file in sorted(csv_files):
                ...
                print(
                    f"   ✅ {csv_file.name} ({size_kb:.1f} KB, {mod_time.strftime('%H:%M:%S')})"
                )
        else:
    ...
    large_video_files = [
        "data/output_videos/FULL_MATCHPortugalvSpain_output.avi",
        "data/output/FULL_MATCHPortugalvSpain_team_stats.csv",
        "data/output/FULL_MATCHPortugalvSpain_player_stats.csv",
    ]
...
def main():
    ...
    print(f"   🎬 Video: data/output_videos/FULL_MATCHPortugalvSpain_output.avi")
    print(f"   📊 Team Stats: data/output/FULL_MATCHPortugalvSpain_team_stats.csv")
    print(f"   📊 Player Stats: data/output/FULL_MATCHPortugalvSpain_player_stats.csv")
