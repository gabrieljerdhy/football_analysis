...
from pass_counter.unified_statistics_manager import (
...
def parse_arguments():
    ...
    parser.add_argument(
        "--output-dir", default="data/output", help="Output directory for results"
    )
    parser.add_argument(
        "--use-enhanced-stats",
        action="store_true",
    ...
    parser.add_argument(
        "--frame-rate", type=float, default=24.0, help="Video frame rate"
    )
    parser.add_argument(
        "--batch-size", type=int, default=20, help="Batch size for processing"
    )
    parser.add_argument(
        "--export-video",
        action="store_true",
...
def load_or_generate_tracks(
    input_video: str, stub_path: Optional[str], batch_size: int
) -> Dict:
...
def assign_teams_and_process_tracking(tracks: Dict, input_video: str) -> Dict:
    ...
    for frame_num, player_track in enumerate(tracks["players"]):
        for player_id, track in player_track.items():
            team = team_assigner.get_player_team(first_frame, track["bbox"], player_id)
    ...
    try:
        ...
        if args.use_enhanced_stats and stats_manager:
            ...
            summary_path = os.path.join(
                args.output_dir, "enhanced_analysis_summary.txt"
            )
        ...
        if args.use_enhanced_stats and stats_manager:
            ...
            if summary:
                ...
                print(
                    f"   • Processing Time: {summary['performance_metrics']['processing_time']:.1f}s"
                )
...
def run_legacy_statistics_analysis(tracks: Dict, output_dir: str):
    ...
    pass_counter.export_player_stats_to_csv(
        os.path.join(output_dir, "player_stats.csv")
    )
    tackle_counter.export_team_stats_to_csv(
        os.path.join(output_dir, "tackle_team_stats.csv")
    )
    tackle_counter.export_player_stats_to_csv(
        os.path.join(output_dir, "tackle_player_stats.csv")
    )
