...
from ..cross_detection import CrossDetector
from .ball_movement_analyzer import BallTrajectorySegment, EnhancedBallMovementAnalyzer
...
class EnhancedPassCounter:
    ...
    def __init__(self, video_width=None, video_height=None, frame_rate=24.0):
        ...
        # Ball position tracking for distance calculation
        self.last_ball_position = None

    def set_field_keypoints_detector(self, detector):
        """Set the field keypoints detector for enhanced cross detection."""
        self.cross_detector.set_field_keypoints_detector(detector)

    def analyze_passes_from_tracks(
        self, tracks: Dict, player_assignments: List[int]
    ) -> Dict:
        """
    ...
    def count_passes_enhanced(
        ...
        # Detect crosses using cross detector
        if ball_data and "position" in ball_data:
            ball_position = ball_data["position"]
            ball_confidence = ball_data.get("confidence", 0.8)
            ball_source = ball_data.get("source", "detection")

            cross_event = self.cross_detector.detect_cross(
                ball_position,
                current_player_id,
                current_team,
                frame_num,
            ...
            if cross_event:
                print(
                    f"🎯 Cross detected: Player {cross_event.player_id}, Team {cross_event.team_id}, "
                    f"Type: {cross_event.cross_type.value}, Confidence: {cross_event.confidence_score:.3f}"
                )
    ...
    def _process_possession_changes(
        self, tracks: Dict, player_assignments: List[int], ball_analysis: Dict
    ):
    ...
    def _handle_possession_change(
        ...
        if (
            new_team_id == self.last_team
            and self.possession_frames >= self.min_possession_frames
        ):
    ...
    def _detect_pass(
        self, receiver_id: int, team_id: int, frame_num: int, ball_data: Dict = None
    ):
        """Detect and validate a pass event with enhanced confidence calculation."""
        if self.last_player_id == -1:
            return

        # Calculate pass distance and velocity with improved accuracy
        pass_distance = 0.0
        pass_velocity = 0.0
        base_confidence = 0.5

        if ball_data:
            base_confidence = ball_data.get("confidence", 0.5)
            pass_velocity = ball_data.get("velocity", 5.0)

            # Get ball position for distance calculation
            if "position" in ball_data and hasattr(self, "last_ball_position"):
                current_pos = ball_data["position"]
                if self.last_ball_position:
                    pass_distance = measure_distance(
                        self.last_ball_position, current_pos
                    )
                else:
                    pass_distance = 50.0  # Default estimate
            else:
                pass_distance = 50.0  # Default estimate

        # Calculate enhanced confidence score
        confidence_score = self._calculate_pass_confidence(
            base_confidence, pass_distance, pass_velocity, receiver_id, team_id
        )

        # Validate pass based on confidence and parameters
        if (
            confidence_score >= self.pass_confidence_threshold
            and self.min_pass_distance <= pass_distance <= self.max_pass_distance
        ):
            ...
            # Update pass confidence tracking
            player_stats = self.player_passes[self.last_player_id]
            player_stats["total_pass_confidence"] += confidence_score
            player_stats["avg_pass_confidence"] = (
                player_stats["total_pass_confidence"] / player_stats["passes"]
            )
            ...
            print(
                f"Enhanced pass detected: Player {self.last_player_id} -> {receiver_id} "
                f"(Team {team_id}) at frame {frame_num}, confidence: {confidence_score:.2f}"
            )

        # Store ball position for next frame distance calculation
        if ball_data and "position" in ball_data:
            self.last_ball_position = ball_data["position"]

    def _calculate_pass_confidence(
        self,
        ...
        """
        Calculate enhanced confidence score for pass detection.

        Args:
            base_confidence (float): Base confidence from ball detection
            distance (float): Pass distance in pixels
            velocity (float): Ball velocity in m/s
            receiver_id (int): ID of receiving player
            team_id (int): Team ID

        Returns:
            float: Enhanced confidence score (0.0 to 1.0)
        """
        # Start with base confidence from ball detection
        confidence = base_confidence

        # Distance factor: optimal pass distances get higher confidence
        if self.min_pass_distance <= distance <= self.max_pass_distance:
            # Normalize distance to 0-1 range
            normalized_distance = (distance - self.min_pass_distance) / (
                self.max_pass_distance - self.min_pass_distance
            )
            # Optimal distance is around 0.3-0.7 of the range
            if 0.2 <= normalized_distance <= 0.8:
                distance_factor = 1.0
            else:
                distance_factor = 0.8
        else:
            distance_factor = 0.3  # Outside optimal range

        # Velocity factor: reasonable velocities get higher confidence
        if self.min_pass_velocity <= velocity <= 15.0:  # 15 m/s is reasonable max
            velocity_factor = 1.0
        elif velocity > 15.0:
            velocity_factor = max(
                0.5, 1.0 - (velocity - 15.0) / 20.0
            )  # Decrease for very high velocity
        else:
            velocity_factor = 0.6  # Very low velocity

        # Team consistency factor: same team passes are more likely
        team_factor = 1.0  # Assume same team for now

        # Combine factors with weights
        enhanced_confidence = (
            confidence * 0.4  # Base detection confidence (40%)
            + distance_factor * 0.3  # Distance appropriateness (30%)
            + velocity_factor * 0.2  # Velocity reasonableness (20%)
            + team_factor * 0.1  # Team consistency (10%)
        )
        ...
        for pass_event in self.pass_events:
            ...
            if matching_segment:
                ...
                pass_event.confidence_score = min(
                    pass_event.confidence_score
                    + matching_segment.confidence_score * 0.3,
                    1.0,
                )
    ...
    def _generate_pass_statistics(self) -> Dict:
        ...
        for team_id in [1, 2]:
            ...
            team_stats[team_id] = {
                ...
                "avg_pass_distance": (
                    np.mean([p.pass_distance for p in team_passes])
                    if team_passes
                    else 0
                ),
                "avg_pass_velocity": (
                    np.mean([p.pass_velocity for p in team_passes])
                    if team_passes
                    else 0
                ),
                "pass_accuracy": (
                    np.mean([p.pass_accuracy for p in team_passes])
                    if team_passes
                    else 0
                ),
                "short_passes": len([p for p in team_passes if p.pass_type == "short"]),
                "medium_passes": len(
                    [p for p in team_passes if p.pass_type == "medium"]
                ),
        ...
        # Get cross statistics
        cross_stats = self.cross_detector.get_cross_statistics()
        ...
        return {
            ...
            "quality_metrics": {
                "avg_confidence": np.mean(
                    [p.confidence_score for p in self.pass_events]
                ),
                "trajectory_validated_passes": len(
                    [p for p in self.pass_events if p.trajectory_segment]
                ),
            },
        }
        ...
        for team_id, possessions in team_possessions.items():
            if possessions:
                ...
                stats[team_id] = {
                    ...
                    "avg_possession_quality": np.mean(
                        [p.possession_quality for p in possessions]
                    ),
                }
            else:
                stats[team_id] = {
